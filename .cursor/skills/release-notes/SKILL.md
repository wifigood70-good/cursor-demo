...
## 절차
1. 커밋 목록을 수집한다.
   - **Windows (PowerShell):** `chmod`는 사용할 수 없다. Git Bash로 실행한다.
     ```powershell
     bash .cursor/skills/release-notes/scripts/collect_commits.sh <이전태그> <현재태그>
     ```
   - **macOS/Linux:** `./.cursor/skills/release-notes/scripts/collect_commits.sh <이전태그> <현재태그>`
     (최초 1회: `chmod +x .cursor/skills/release-notes/scripts/collect_commits.sh`)
